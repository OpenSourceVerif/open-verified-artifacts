...
# Z3 formal verification for cnum32_lshr_const
# Theorem: forall ci, k, x: ci non-empty and x in ci and k < 32
#         implies (x >> k) in cnum32_lshr_const(ci, k)
#
...
#                   else -> top
#   from_urange(min,max): if min > max -> EMPTY; else {base=min, size=max-min}
...
def main():
    ...
    # cross_unsigned_limit: only called when !ci_top in C,
    ...
    res_base = If(Or(is_empty(ci_base, ci_size), ci_top),
                  ci_base,  # EMPTY -> ci_base (still EMPTY), top -> ci_base (unchanged)
                  If(k_large,
                     BitVecVal(0, 32),
                     If(ci_crosses,
                        BitVecVal(0, 32),
    ...
    res_size = If(Or(is_empty(ci_base, ci_size), ci_top),
                  ci_size,  # EMPTY -> ci_size (still EMPTY), top -> ci_size (unchanged)
                  If(k_large,
                     UT_MAX,
                     If(ci_crosses,
                        UT_MAX,
    ...
    print("Theorem: x in ci and k < 32 implies (x>>k) in lshr_const(ci,k)\n")
