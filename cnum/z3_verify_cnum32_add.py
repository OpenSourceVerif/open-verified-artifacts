...
# Z3 formal verification for cnum32_add
# Theorem: forall ci, cj, x, y: ci non-empty and cj non-empty
#         and x in ci and y in cj implies x+y in cnum32_add(ci, cj)
#
# C implementation (from cnum_def.h, T=32):
#   is_empty:   base == 0 && size == 0
...
def main():
    ...
    cj_base = BitVec("cj_base", 32)
    cj_size = BitVec("cj_size", 32)
    x = BitVec("x", 32)
    y = BitVec("y", 32)
    ...
    print("Verifying cnum32_add (1hr timeout)...")
    print("Theorem: x in ci and y in cj implies x+y in cnum32_add(ci,cj)\n")
    ...
    if result == sat:
        ...
        print("PASS: cnum32_add verified.")
        print(f"  elapsed: {elapsed:.3f}s")
        return 0
    else:
