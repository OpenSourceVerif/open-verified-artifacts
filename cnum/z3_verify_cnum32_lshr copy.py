...
def main():
    ...
    if result == sat:
        ...
        m = s.model()
        print(f"  ci  = {{base={m[ci_base].as_long():#010x}, size={m[ci_size].as_long():#010x}}}")
        print(f"  cx  = {{base={m[cx_base].as_long():#010x}, size={m[cx_size].as_long():#010x}}}")
        print(f"  x   = {m[x].as_long():#010x}")
        print(f"  k   = {m[k].as_long():#010x}")
        xv = m[x].as_long()
        kv = m[k].as_long()
        print(f"  x>>k = {xv >> kv:#010x}")
        print(f"  res = {{base={m[res_base].as_long():#010x}, size={m[res_size].as_long():#010x}}}")
        return 1
    elif result == unsat:
