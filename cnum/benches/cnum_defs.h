 ...
 * If cnum represents a range crossing the ST_MAX/ST_MIN boundary, the unbound
 * range [ST_MIN..ST_MAX] is returned.
 */
...
st FN(smin_new)(struct cnum_t cnum)
{
...
st FN(smax_new)(struct cnum_t cnum)
{
...
struct cnum_t FN(normalize)(struct cnum_t cnum)
{
...
struct cnum_t FN(add)(struct cnum_t a, struct cnum_t b)
{
	...
	if (a.size > UT_MAX - b.size)
		...
		return FN(normalize)(
			(struct cnum_t){ a.base + b.base, a.size + b.size });
}
...
bool FN(contains_new)(struct cnum_t cnum, ut v)
{
...
bool FN(is_subset)(struct cnum_t bigger, struct cnum_t smaller)
{
	...
	if (FN(is_empty(bigger)))
		return false;
	/* rotate both arcs such that 'bigger' starts at origin, hence does not
   * overflow */
...
char *FN(fmt)(struct cnum_t c)
{
#if T == 32
#define FMT "{ .base = %u, .size = %u }"
#else
#define FMT "{ .base = %lu, .size = %lu }"
#endif // T == 32

	char *str = NULL;
	if (asprintf(&str, FMT, c.base, c.size) == -1)
		perror("error while formatting cnum");
