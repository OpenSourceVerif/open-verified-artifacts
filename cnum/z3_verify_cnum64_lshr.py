...
# Z3 formal verification for cnum64_lshr
# Theorem: forall ci, cx, x, k:
#         ci non-empty, cx non-empty, x in ci, k in cx, cx constant
#         implies (x >> k) in cnum64_lshr(ci, cx)
#
# C implementation (from cnum_def.h, T=64):
#   is_const:  size == 0
#   lshr_const(ci,k):  see z3_verify_cnum64_lshr_const.py
...
def main():
    ...
    cx_base = BitVec("cx_base", 64)
    cx_size = BitVec("cx_size", 64)
    x = BitVec("x", 64)
    k = BitVec("k", 64)
    ...
    k_large = UGE(cx_base, BitVecVal(64, 64))
    ...
    print("Verifying cnum64_lshr (1hr timeout)...")
    ...
    if result == sat:
        ...
        print("PASS: cnum64_lshr verified.")
        print(f"  elapsed: {elapsed:.3f}s")
        return 0
    else:
