...
// bswap_signed_simple: 同号使用原始算法，不同号返回 top
std::pair<expr, expr> bswap_signed_simple(context& c, expr x, expr y) {
    expr zero = c.bv_val(0, 64);
    
    // 检查是否同号
    // 同号: (x < 0 && y < 0) || (x >= 0 && y >= 0)
    expr x_negative = x < zero;
    expr y_negative = y < zero;
    expr x_non_negative = x >= zero;
    expr y_non_negative = y >= zero;
    
    expr same_sign = (x_negative && y_negative) || (x_non_negative && y_non_negative);
    
    // 如果同号，使用原始算法（直接调用 bswap_min_signed 和 bswap_max_signed）
    expr same_sign_min = bswap_min_signed(c, x, y);
    expr same_sign_max = bswap_max_signed(c, x, y);
    

    expr top_min = c.bv_val((uint64_t)0x8000000000000000ULL, 64);
    expr top_max = c.bv_val((uint64_t)0x7FFFFFFFFFFFFFFFULL, 64);
    
    expr result_min = ite(same_sign, same_sign_min, top_min);
    expr result_max = ite(same_sign, same_sign_max, top_max);
...
int main() {
    ...
    auto [algo_min, algo_max] = bswap_signed_simple(c, lb, ub);
    std::cout << "完成\n\n";
    ...
    if (result == sat) {
        ...
        // 检查是否同号
        expr zero = c.bv_val(0, 64);
        expr lb_neg = m.eval(lb < zero);
        expr ub_neg = m.eval(ub < zero);
        std::cout << "\n符号信息：\n";
        std::cout << "  lb < 0: " << lb_neg << "\n";
        std::cout << "  ub < 0: " << ub_neg << "\n";
